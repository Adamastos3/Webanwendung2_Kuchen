...
class BestellpositionDao {
  ...
  loadById(id) {
    ...
    result.mehrwertsteuersumme = helper.round(
      result.menge * result.produkt.mehrwertsteueranteil
    );
    result.nettosumme = helper.round(result.menge * result.produkt.nettopreis);
    result.bruttosumme = helper.round(
      result.menge * result.produkt.bruttopreis
    );
  ...
  loadAll() {
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      result[i].mehrwertsteuersumme = helper.round(
        result[i].menge * result[i].produkt.mehrwertsteueranteil
      );
      result[i].nettosumme = helper.round(
        result[i].menge * result[i].produkt.nettopreis
      );
      result[i].bruttosumme = helper.round(
        result[i].menge * result[i].produkt.bruttopreis
      );
    }
  ...
  update(id, bestellungid = 1, produktid = 1, menge = 1) {
    var sql =
      "UPDATE Bestellposition SET BestellungID=?,ProduktID=?,Menge=? WHERE ID=?";
    var statement = this._conn.prepare(sql);
  ...
  deleteByParent(bestellungid) {
    try {
      ...
      throw new Error(
        "Could not delete Records by bestellungid=" +
          bestellungid +
          ". Reason: " +
          ex.message
      );
