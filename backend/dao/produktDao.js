...
class ProduktDao {
  ...
  loadById(id) {
    ...
    console.log(result.nettopreis);
    console.log(result.mehrwertsteuer.steuersatz);
  ...
  create(
    ...
    var sql =
      "INSERT INTO Produkt (KategorieID,Bezeichnung,Beschreibung,MehrwertsteuerID,Details,Nettopreis,DatenblattID) VALUES (?,?,?,?,?,?,?)";
    var statement = this._conn.prepare(sql);
    var params = [
      kategorieid,
      bezeichnung,
  ...
  update(
    ...
    var sql =
      "UPDATE Produkt SET KategorieID=?,Bezeichnung=?,Beschreibung=?,MehrwertsteuerID=?,Details=?,Nettopreis=?,DatenblattID=?, Geloescht=? WHERE ID=?";
    var statement = this._conn.prepare(sql);
    var params = [
      kategorieid,
      bezeichnung,
