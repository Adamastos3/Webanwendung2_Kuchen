...
serviceRouter.post("/adresse", function (request, response) {
  ...
  var errorMsgs = [];
  if (helper.isUndefined(request.body.strasse)) errorMsgs.push("strasse fehlt");
...
serviceRouter.put("/adresse", function (request, response) {
  ...
  if (helper.isUndefined(request.body.strasse)) errorMsgs.push("strasse fehl");
  if (helper.isUndefined(request.body.hausnummer))
...
serviceRouter.delete("/adresse/:id", function (request, response) {
  ...
  try {
    ...
    helper.log(
      "Service Adresse: Deletion of record successfull, id=" + request.params.id
    );
    response
      .status(200)
    ...
    helper.logError(
      "Service Adresse: Error deleting record. Exception occured: " + ex.message
    );
    response.status(400).json(helper.jsonMsgError(ex.message));
  }
});
