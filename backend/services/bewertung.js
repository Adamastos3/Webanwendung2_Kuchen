...
serviceRouter.get("/bewertung/gib/:id/:zugang", function (request, response) {
  helper.log(
    "Service Bewertung: Client requested one record, id=" + request.params.id
  );
  if (auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)) {
    ...
    try {
      ...
      helper.logError(
        "Service Bewertung: Error loading record by id. Exception occured: " +
          ex.message
      );
...
serviceRouter.get("/bewertung/alle/:zugang", function (request, response) {
  ...
  if (auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)) {
    ...
    try {
      ...
      helper.logError(
        "Service Bewertung: Error loading all records. Exception occured: " +
          ex.message
      );
      response.status(400).json(helper.jsonMsgError(ex.message));
    }
...
serviceRouter.get(
  "/bewertung/gericht/:id/:zugang",
  function (request, response) {
    helper.log(
      "Service Bewertung: Client requested all record for parent, id=" +
        request.params.id
    );
    if (
      ...
      try {
        ...
        helper.logError(
          "Service Bewertung: Error loading all records for parent by id. Exception occured: " +
            ex.message
        );
        response.status(400).json(helper.jsonMsgError(ex.message));
      }
...
serviceRouter.get(
  "/bewertung/existiert/:id/:zugang",
  function (request, response) {
    helper.log(
      "Service Bewertung: Client requested check, if record exists, id=" +
        request.params.id
    );
    if (
      auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)
      ...
      try {
        ...
        helper.log(
          "Service Bewertung: Check if record exists by id=" +
            request.params.id +
            ", result=" +
            result
        );
        ...
        helper.logError(
          "Service Bewertung: Error checking if record exists. Exception occured: " +
            ex.message
        );
        response.status(400).json(helper.jsonMsgError(ex.message));
      }
...
serviceRouter.post("/bewertung/:zugang", function (request, response) {
  ...
  if (auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)) {
    ...
    if (helper.isUndefined(request.body.zeitpunkt)) {
      ...
      errorMsgs.push(
        "zeitpunkt hat das falsche Format, erlaubt: dd.mm.jjjj hh:mm:ss"
      );
    } else {
      request.body.zeitpunkt = helper.parseGermanDateTimeString(
        request.body.zeitpunkt
      );
    }
    ...
    const bewertungDao = new BewertungDao(request.app.locals.dbConnection);
    try {
      var result = bewertungDao.create(
        request.body.gericht.id,
        request.body.punkte,
        request.body.zeitpunkt,
        request.body.bemerkung,
        request.body.ersteller
      );
      helper.log("Service Bewertung: Record inserted");
      response.status(200).json(helper.jsonMsgOK(result));
    } catch (ex) {
      helper.logError(
        "Service Bewertung: Error creating new record. Exception occured: " +
          ex.message
      );
      response.status(400).json(helper.jsonMsgError(ex.message));
    }
...
serviceRouter.put("/bewertung/:zugang", function (request, response) {
  ...
  if (auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)) {
    ...
    const bewertungDao = new BewertungDao(request.app.locals.dbConnection);
    try {
      var result = bewertungDao.update(
        request.body.id,
        request.body.gericht.id,
        request.body.punkte,
        request.body.zeitpunkt,
      ...
      helper.logError(
        "Service Bewertung: Error updating record by id. Exception occured: " +
          ex.message
      );
      response.status(400).json(helper.jsonMsgError(ex.message));
    }
...
serviceRouter.delete("/bewertung/:id/:zugang", function (request, response) {
  helper.log(
    "Service Bewertung: Client requested deletion of record, id=" +
      request.params.id
  );
  if (auth.checkAuth(request.app.locals.dbConnection, request.params.zugang)) {
    const bewertungDao = new BewertungDao(request.app.locals.dbConnection);
    try {
      ...
      helper.log(
        "Service Bewertung: Deletion of record successfull, id=" +
          request.params.id
      );
      response
        .status(200)
      ...
      helper.logError(
        "Service Bewertung: Error deleting record. Exception occured: " +
          ex.message
      );
      response.status(400).json(helper.jsonMsgError(ex.message));
    }
